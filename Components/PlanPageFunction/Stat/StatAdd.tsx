...
const StatAdd = () => {
    ...
    const minutes = planToday.reduce((total, current) => total + current.duration, 0)
    const calories = planToday.reduce((total, current) => total + current.caloriesBurned, 0)
    return (
        <div className='flex flex-col sm:flex-row justify-start items-center gap-5 sm:gap-0 w-full rounded-2xl p-6 bg-[#13161d] border border-[#232732]'>
            <div className='flex flex-col justify-between items-start w-full'>
                ...
                <p className='text-[#ccff00] font-oswald text-4xl font-bold'>{planToday.length}</p>
            </div>
